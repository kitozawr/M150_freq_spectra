...
def do_processing_plot(window, mode):
    ...
    def FAS_2D_folder():
        ...
        for i, file in enumerate(os.listdir(pathname)):
            ...
            if (file.endswith(
                    filename_extension)):  # and dictionary_of_match.get(i) and dictionary_of_match.get(i)[1] > 0):
                try:
                    if file.endswith(".png"):
                        PB.do_image_to_array('', pathname + "/" + file)
                    elif file.endswith(".dat"):
                    ...
                    for i, j in zip(coordinates[:, 0], coordinates[:, 1]):
                        ...
                        if array[i, j] > 0.02*MAX and freq_array[j] > 820 and 400 < i < 500:
                            if True:
                                print(np.amax(wf0) * m + c, time_pb, freq_array[j], array[i, j])
                except:
    ...
    def FAS_3D_en():
        ...
        if pathname:
            for file in os.listdir(pathname):
                if file.endswith(filename_extension):
                    energy = float(file[:file.find("_")])
                    if energy_max >= energy >= energy_min:
                        if file.endswith(".png"):
                            PB.do_image_to_array('', pathname + "/" + file)
                        elif file.endswith(".dat"):
                            PB.do_data_to_array('', pathname + "/" + file)
    ...
    def FAS_3D_dist():
        ...
        for pathname in [
            ...
            'F:/Филаментация/Спектры/2019/август/2_matched'
            '/F3m_AM_no_lambda_250cm_815nm_ns10_mode_f850nm_ns12P_10old_gain10',
            'F:/Филаментация/Спектры/2019/август/2_matched'
            '/F3m_AM_no_lambda_260cm_815nm_ns10_mode_f850nm_ns12_10old_gain10_true',
            'F:/Филаментация/Спектры/2019/август/2_matched'
            '/F3m_AM_no_lambda_270cm_815nm_ns10_mode_f850nm_ns12_10old_gain10_real_true',
            'F:/Филаментация/Спектры/2019/август/2_matched'
            '/F3m_AM_no_lambda_280cm_815nm_ns10_mode_f850nm_ns12_10old_gain10_real_true',
            'F:/Филаментация/Спектры/2019/август/2_matched'
            '/F3m_AM_no_lambda_290cm_815nm_ns10_mode_f850nm_ns12P_10old_gain10',
            ...
            'F:/Филаментация/Спектры/2019/август/1_matched('
            'using2augcalibration)/F3m_AM_no_lambda_310cm_810nm_ns10_mode_f850nm_ns12_gain20',
            'F:/Филаментация/Спектры/2019/август/1_matched('
            'using2augcalibration)/F3m_AM_no_lambda_320cm_810nm_ns10_mode_f850nm_ns12_gain20_true',
            'F:/Филаментация/Спектры/2019/август/1_matched('
            'using2augcalibration)/F3m_AM_no_lambda_330cm_810nm_ns10_mode_f850nm_ns12_gain20_true',
            'F:/Филаментация/Спектры/2019/август/1_matched('
            'using2augcalibration)/F3m_AM_no_lambda_340cm_810nm_ns10_mode_f850nm_ns12P_gain20',
            'F:/Филаментация/Спектры/2019/август/1_matched('
            'using2augcalibration)/F3m_AM_no_lambda_350cm_810nm_ns10_mode_f850nm_ns12P_gain20']:
    ...
    def finding_local_maxima(show=True):
        ...
        def display_results():
            ...
            for x, y in zip(coordinates[:, 0], coordinates[:, 1]):
                if im[x, y] > 0.02*MAX and 820 < freq[y] < 900:
                    ax[1].plot(y, x, 'w.')
                    # print(y, freq[y])
            ax[1].axis('off')
        ...
        if show:
            display_results()
        for x, y in zip(coordinates[:, 0], coordinates[:, 1]):
            if im[x, y] > 0.02 and 820 < freq[y] < 900:
                return freq[y]
